...
console.log(`There are some useful constants like pi (${Math.PI.toFixed(4)})`+
            ` or e (${Math.E.toFixed(4)}).`);

let num3 = 8.49;
console.log(`Ceil, floor, round of ${num3} are: ${Math.ceil(num3)}, ` +
            `${Math.floor(num3)}, ${Math.round(num3)}.`);
console.log(`If you were to round to the first decimal place, ` +
            `you need to use some tricks (Math.round(num3*10)/10) ` +
            `-> ${Math.round(num3*10)/10}.`);

let num4 = Number('x');
console.log(`num4 is NaN (${num4}). With neither == nor ===, this can be checked. ` +
            `== (${num4==NaN}), === (${num4===NaN}). ` +
            `isNaN is the only way to figure. (${isNaN(num4)})`);

let num5 = '123...just kidding this is a string';
console.log(`parseInt reads the input upto number part and returns integer. ` +
            `num5 (${num5}) -> parseInt(num5) (${parseInt(num5)}).`)
console.log(`Second arg can specify number system. (FF -> ${parseInt('ff', 16)})`)

let num6 = '123.4567..just kidding this is a string';
console.log(`parseFloat works similarly. ` +
...
console.log(`max, min, pow, abs, sqrt are useful, too. ` +
